...
pub fn file_to_acf(file: &str) -> Result<AcfValue, Box<Error<Rule>>> {
    // Just panic here if the file is invalid
    let acf = AcfParser::parse(Rule::collection, file)?
        .next()
        .expect("Acf file provided could not be read");
    ...
    fn parse_value(pair: Pair<Rule>) -> AcfValue {
        match pair.as_rule() {
            Rule::pair => {
                ...
                AcfValue::Pair(name, result)
            }
            Rule::collection => {
                let mut inner_rules = pair.into_inner();
                let name = inner_rules.next().unwrap().as_str().trim_matches('\"');

                let contents = inner_rules.map(parse_value).collect();

                AcfValue::Collection(name, contents)
            }
            _ => AcfValue::Null,
        }
    }
            ...
            body.into_iter().for_each(|workshop| {
                if let AcfValue::Collection(name, metadata) = workshop {
                    ...
                    if let Some(AcfValue::Pair(_, value)) = result {
                        workshops.insert(name, value.parse::<u64>().unwrap());
                    }
                }
            })
        }
